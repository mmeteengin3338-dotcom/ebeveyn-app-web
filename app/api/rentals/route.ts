...
export async function GET(req: Request) {
  try {
    ...
    const rentals = ((data ?? []) as RentalRow[]).map((item) => ({
      ...item,
      status: normalizeRentalStatus(String(item?.status ?? "")) ?? String(item?.status ?? ""),
    }))
...
export async function POST(req: Request) {
  try {
    ...
    const offer_note = asTrimmedString(body.offer_note)
    ...
    let insertResult = await supabase.from("rentals").insert(insertPayload).select("*").single()

    if (insertResult.error && /owner_email/i.test(insertResult.error.message)) {
      const fallbackPayload = { ...insertPayload }
      delete fallbackPayload.owner_email
      insertResult = await supabase.from("rentals").insert(fallbackPayload).select("*").single()
    }
    ...
    if (ownerEmail && renterEmail && ownerEmail !== renterEmail) {
      const noteText = offer_note ? ` | ${offer_note}` : ""
      const notifyText = `Yeni kiralama teklifi: ${prod.title} (${start_date} - ${end_date})${noteText}`
